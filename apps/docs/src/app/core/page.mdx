...
`CellData.width` follows terminal column semantics: `2` is the leading cell for
CJK/fullwidth characters, `0` is its spacer cell, and `1` is a normal cell.
Custom renderers should skip spacer cells and advance by the reported width.

`getScrollbackGeneration()` returns a monotonically increasing version for
scrollback content changes. Use it with `getScrollbackCount()` when caching
expensive scrollback projections: live grid updates do not change the
generation, but appended, dropped, or reset scrollback rows do.
